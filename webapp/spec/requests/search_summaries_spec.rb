  ...
  let(:cocina_json) do
    {
      ...
      description: { title: [ { value: "Frogs" } ], note: [ { type: "abstract", value: "Frogs of California." } ] }
    }.to_json
  end

  it "renders the summary above results without calling the AI service during page load" do
    docs = [ { "id" => "abc", "title_display_tesi" => "Frogs", "cocina_ss" => cocina_json } ]
    solr_response = Blacklight::Solr::Response.new(
      { "response" => { "numFound" => 1, "start" => 0, "docs" => docs } },
